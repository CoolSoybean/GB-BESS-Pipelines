  ...
  return <div className="shell">
    ...
    <main>
      ...
      <section id="projects" className="panel projects-panel">
        ...
        <div className="table-wrap"><table><thead><tr><th>Project</th><th>Connection</th><th>Status</th><th>Capacity</th><th>Target</th><th>Operator</th></tr></thead><tbody>{projects.map(p=><tr key={p.id}><td><strong>{p.project_name}</strong><span>{p.site_name}</span></td><td><span className={`connection ${p.source}`}><i/>{p.source}</span></td><td><StatusPill value={p.status}/></td><td><strong>{formatCapacity(p.capacity_mw)}</strong></td><td>{p.target_year ?? '—'}</td><td>{p.operator_name ?? '—'}</td></tr>)}{!projects.length&&<tr><td colSpan={6} className="empty">No projects match these filters.</td></tr>}</tbody></table></div>
      </section>
